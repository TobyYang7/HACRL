...
1. `docs/training_pipeline.md` 是当前 train/eval pipeline 的 canonical 文档。
2. `mydocs/` 是仓库知识库，用于解释与导航，不替代 canonical 文档。
3. 若 `mydocs` 与 `docs/training_pipeline.md` 不一致，以 `docs/training_pipeline.md` 为准，并同步修订 `mydocs`。
...
1. 先判断任务是否能拆成 2 个及以上互不冲突的子任务。
2. 若能拆分，优先让主 agent 负责统筹和最终整合，subagent 负责事实梳理、草稿编写、局部实现或验证。
3. 若不能安全拆分，再由主 agent 顺序完成。
